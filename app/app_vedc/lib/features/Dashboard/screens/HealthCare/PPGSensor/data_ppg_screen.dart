...
class DataPpgScreen extends StatefulWidget {
  const DataPpgScreen({super.key, this.lastCsvPath});

  final String? lastCsvPath;

  @override
  State<DataPpgScreen> createState() => _DataPpgScreenState();
}

class _DataPpgScreenState extends State<DataPpgScreen> {
  String? _uploadingPath;
  String? _sendingPath;
  late Future<Map<String, Object>> _filesFuture;

  static const String _filePrefix = 'ppg_';
  ...
  Future<void> _deleteFileWithConfirm(
    ...
    try {
      // ignore: avoid_print
      print('Attempting to delete PPG file: ${file.path}');
      ...
      // ignore: avoid_print
      print('Deleted PPG file: ${file.path}');
      if (!mounted) return;
      ScaffoldMessenger.of(
        context,
  ...
  Future<void> _uploadToFirestore(File file) async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('ppg_exports').add({
        'fileName': fileName,
        'sizeBytes': size,
  ...
  Future<void> _sendEmail(File file) async {
    ...
    try {
      ...
      final subject = '[VEDC] Dữ liệu PPG - $fileName';
      final body =
          '''Kính gửi Bác sĩ,

Tôi gửi kèm file dữ liệu PPG (tên file: $fileName) thu thập từ ứng dụng VEDC.
