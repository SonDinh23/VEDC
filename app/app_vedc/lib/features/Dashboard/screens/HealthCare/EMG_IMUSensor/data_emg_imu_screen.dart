...
class DataEmgImuScreen extends StatefulWidget {
  const DataEmgImuScreen({super.key, this.lastCsvPath});

  final String? lastCsvPath;

  @override
  State<DataEmgImuScreen> createState() => _DataEmgImuScreenState();
}

class _DataEmgImuScreenState extends State<DataEmgImuScreen> {
  String? _uploadingPath;
  String? _sendingPath;

  static const String _filePrefix = 'emg_imu_';
  ...
  Future<void> _deleteFileWithConfirm(
    ...
    try {
      // Debug log
      // ignore: avoid_print
      print('Attempting to delete file: ${file.path}');

      final exists = await file.exists();
      if (!exists) {
        // If the file is already missing, still attempt to remove Firestore doc
      ...
      // If synced in Firestore, remove document as well
      final docId = syncedMap[name];
      if (docId != null) {
        await FirebaseFirestore.instance
            .collection('emg_imu_exports')
      ...
      // Log and show detailed error to help debug on device/emulator
      // ignore: avoid_print
  ...
  Future<void> _uploadToFirestore(File file) async {
    ...
    try {
      ...
      // Firestore giới hạn ~1MB/document; cảnh báo nếu lớn.
      if (size > 900 * 1024) {
        throw StateError(
      ...
      await FirebaseFirestore.instance.collection('emg_imu_exports').add({
        'fileName': fileName,
        'sizeBytes': size,
  ...
  Future<void> _sendEmail(File file) async {
    ...
    try {
      ...
      final subject = '[VEDC] Dữ liệu EMG & IMU - $fileName';
      final body =
          '''Kính gửi Bác sĩ,

Tôi gửi kèm file dữ liệu EMG & IMU (tên file: $fileName) thu thập từ ứng dụng VEDC.
